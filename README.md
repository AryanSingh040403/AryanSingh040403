# Aryan Singh

**CSE(AIML) Eng. Graduate | Machine Learning & Generative AI Engineer (LLMs, NLP, RAG)**  
Building intelligent, data-driven systems with a focus on Large Language Models, NLP pipelines, and applied Deep Learning.

---

## About Me
- CSE(AIML) graduate with strong foundations in **Machine Learning, Deep Learning, and Natural Language Processing**
- Hands-on experience designing and fine-tuning **LLM-based systems**, including **RAG pipelines** and end-to-end ML workflows
- Comfortable working with **research-oriented code** and translating ideas into **production-ready solutions**
- Actively growing through **open-source contributions** and advanced applied AI projects

---

## Tech Stack

### Languages
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-003B57?style=flat&logo=postgresql&logoColor=white)

### Machine Learning & Deep Learning
![PyTorch](https://img.shields.io/badge/PyTorch-EE4C2C?style=flat&logo=pytorch&logoColor=white)
![TensorFlow](https://img.shields.io/badge/TensorFlow-FF6F00?style=flat&logo=tensorflow&logoColor=white)
![scikit-learn](https://img.shields.io/badge/scikit--learn-F7931E?style=flat&logo=scikit-learn&logoColor=white)
![XGBoost](https://img.shields.io/badge/XGBoost-EC4E20?style=flat)

### NLP & Generative AI
![Transformers](https://img.shields.io/badge/Transformers-FFD21E?style=flat&logo=huggingface&logoColor=black)
![Hugging Face](https://img.shields.io/badge/Hugging%20Face-FFD21E?style=flat&logo=huggingface&logoColor=black)
![LLMs](https://img.shields.io/badge/LLMs-000000?style=flat)
![LoRA](https://img.shields.io/badge/LoRA-4B8BBE?style=flat)
![PEFT](https://img.shields.io/badge/PEFT-2C2D72?style=flat)
![RAG](https://img.shields.io/badge/RAG-6A5ACD?style=flat)
![FAISS](https://img.shields.io/badge/FAISS-0467DF?style=flat)

### Data & Visualization
![NumPy](https://img.shields.io/badge/NumPy-013243?style=flat&logo=numpy&logoColor=white)
![Pandas](https://img.shields.io/badge/Pandas-150458?style=flat&logo=pandas&logoColor=white)
![Matplotlib](https://img.shields.io/badge/Matplotlib-11557C?style=flat)
![Seaborn](https://img.shields.io/badge/Seaborn-4C72B0?style=flat)

### MLOps & Development
![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=flat&logo=github&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat&logo=docker&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=flat&logo=linux&logoColor=black)
![Colab](https://img.shields.io/badge/Google%20Colab-F9AB00?style=flat&logo=googlecolab&logoColor=black)

---

## Featured Projects
**YouTube Video Title & Tags Generator (NLP)**  
Transformer-based NLP system for generating SEO-optimized YouTube titles and tags.

**LLM Fine-Tuning for Medical MCQ Question Answering**  
Fine-tuned large language models on medical QA datasets with accuracy and macro-F1 evaluation.

**Intelligent Voice Assistant (LLM + Speech)**  
Conversational voice assistant integrating speech recognition, LLM reasoning, and text-to-speech pipelines.

**Global Pollution Analysis & Energy Recovery**  
Data-driven analysis of global pollution trends with predictive modeling and actionable insights.

➡️ See pinned repositories below for details.

---

## Currently Learning
- Advanced LLM fine-tuning and alignment techniques  
- Retrieval-Augmented Generation (RAG) optimization  
- MLOps, model deployment, and monitoring  
- Open-source collaboration workflows

---

## Open To
- Open-source contributions in Machine Learning and Generative AI  
- Research and engineering collaborations  
- AIML / GenAI Engineer roles and internships

---

## Contact
- 📧 Email: **aryansingh20030404@gmail.com**
- 💼 LinkedIn: https://www.linkedin.com/in/aryansingh0404/
- 🤗 Hugging Face: https://huggingface.co/aryan0404
